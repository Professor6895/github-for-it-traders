...
(function ($) {
  ...
  $('.mobile-search').on('click', function () {
    $('.search-overlay').show();
    setTimeout(function() {
      $('.website-product__search-mobile').focus()
    }, 500)
  });
  ...
  function productSliderNavSetting() {
    return {
      vertical: false,
      slidesToShow: 5,
      slidesToScroll: 1,
  ...
  $('.product-slick').slick(productSlickSetting());
  $('.slider-nav').slick(productSliderNavSetting());

  //PRODUCT COLOR CHANGE AJAX CALL
  $(document).on('click', '.product--variant-handle', function () {
    let variant = $(this).data('variant');

    if (variant === 'color' || variant === 'Color') {
      var color = $(this).data('variant-value');
      ...
      $.ajax({
        url: '/ajax/product/get-variant-image',
        type: 'GET',
        data: payload,
        success: function (data) {
          $('.product--image--wrapper').empty().html(data);

          setTimeout(function () {
            if ($('.product-slick').hasClass('slick-initialized')) {
              $('.product-slick').slick('destroy');
            }
  ...
  //TOGGLERS
  $(document).on('click', '.website--cart-toggler', function () {
    $('#cart_side').toggleClass('open-side')
  })

  $(document).on('click', '.website--wishlist-toggler', function () {
    $('#cart_side').toggleClass('open-side')
  })

  $(document).on('click', '.website--profile-toggler', function () {
    $('#myAccount').toggleClass('open-side')
  })

  $(document).on('click', '.website--login-toggler', function () {
    $('.website--register-modal').modal('hide')
    $('.website--login-modal').modal('show')
  })
  $(document).on('click', '.website--register-toggler', function () {
    $('.website--login-modal').modal('hide')
    $('.website--register-modal').modal('show')
  })
  ...
  $('.social_share').each(function () {
    ...
    $(this).ShareLink({
      title: title, // title for share message
      text: text, // text for share message
      image: image, // optional image for share message (not for all networks)
      url: url, // link on shared page
  ...
  function initHomePageSlick(obj) {
    $(obj).slick({
      dots: false,
      infinite: true,
      speed: 300,
  ...
  $('.home-page-lazy-content').lazy({
    displayLoader: function (element) {
      ...
      $.ajax({
        ...
        var item_slider = element.find('.home-product-slider');
        element.removeAttr('data-src');
        element.removeAttr('data-method');

        if (item_slider[0])
          initHomePageSlick(item_slider);
      });
    }
  })
})(jQuery);
