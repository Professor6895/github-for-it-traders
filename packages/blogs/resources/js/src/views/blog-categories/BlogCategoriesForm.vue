  ...
  <div class="vs-con-loading__container" id="loading--container">
    <div class="tab-text vx-row">
        ...
        <vx-card>
          <div class="vx-row">
            ...
            <div class="vx-col w-full mb-4">
              <vs-input
                name="short_desc"
                v-model="blogCategory.short_desc"
                class="w-full"
      ...
      <div class="vx-col md:w-1/3 w-full mb-4">
        <vx-card title="Blog Category Thumbnail">
          <div class="w-full mb-4">
            ...
            <vs-button
              ...
              type="border"
              size="small"
              @click="$refs.blogCategory_thumbnail_image.click()"
            >Thumbnail Image</vs-button>
          </div>
        </vx-card>
        <vx-card class="mt-3" title="SEO Meta">
          <MetaFields v-model="blogCategory.meta" small></MetaFields>
        </vx-card>
      </div>
      <div class="vx-col w-full">
        <vs-button type="relief" color="primary" @click="saveBlogCategory">Save</vs-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async fetchBlogCategory() {
      ...
      try {
        let response = await this.$http.get(
          `/api/admin/blog-categories/${this.id}`
        );
        if (response.data.success) {
          this.blogCategory = response.data.data;
          this.thumbnail_preview = this.blogCategory.thumbnail_image.thumb;
        }
      } catch (error) {
    ...
    async saveBlogCategory() {
      ...
      try {
        ...
        let params = {
          ...
          content: this.blogCategory.content,
          short_desc: this.blogCategory.short_desc,
          thumbnail: this.blogCategory.thumbnail,
          meta: JSON.stringify(this.blogCategory.meta)
        };

        let url = "/api/admin/blog-categories";
        if (this.id) {
          url = `/api/admin/blog-categories/${this.id}`;
    ...
    imageUploaded() {
      let file = this.$refs.blogCategory_thumbnail_image.files[0];
